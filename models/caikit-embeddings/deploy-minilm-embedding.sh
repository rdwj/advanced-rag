#!/bin/bash
# Deploy all-MiniLM-L6-v2 Embedding model to OpenShift
#
# Prerequisites:
...
echo "Deploying all-MiniLM-L6-v2 Embedding to namespace: $NAMESPACE"
...
oc apply -f "$SCRIPT_DIR/manifests/minilm-embedding/inference-service.yaml" -n "$NAMESPACE"
...
oc apply -f "$SCRIPT_DIR/manifests/minilm-embedding/service.yaml" -n "$NAMESPACE"
oc apply -f "$SCRIPT_DIR/manifests/minilm-embedding/route.yaml" -n "$NAMESPACE"

# Get external endpoint
ROUTE_HOST=$(oc get route all-minilm-l6-v2 -n "$NAMESPACE" -o jsonpath='{.spec.host}')
ENDPOINT="https://$ROUTE_HOST"
echo ""
echo "all-MiniLM-L6-v2 Embedding deployed successfully!"
